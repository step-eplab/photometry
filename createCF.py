...
###############################################################################
def run(config_name, dir_data, dir_save, img_size_X, img_size_Y):
    ...
    if scale_mode:
        ...
        if not MDark_here:
            ...
            MBias_here = os.path.isfile(MBias_path)
            if not MBias_here:
                bias_files = glob.glob(dir_data + f'**/*{bias_template}*.fit', recursive=True)
                if len(bias_files)>0:
                ...
                print('M_BIAS exists')
                M_bias = read_fits(file=MBias_path, Stack=[])[0]
            
            MDark_path_scl = dir_master + f'M_{dark_template_scl}.fits'
            M_dark_scl = read_fits(file=MDark_path_scl, Stack=[])[0]
